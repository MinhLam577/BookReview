    ...
    <div className="bg-white p-6 rounded-3xl border border-black/5 shadow-sm">
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="font-bold text-lg leading-none">{review.username}</h4>
            <p className="text-xs text-brand-primary/40 mt-1">{review.date}</p>
          </div>
        </div>
        <RatingStars rating={review.rating} size={14} className="opacity-90" />
      </div>

      {book && (
        <div className="mb-4 flex items-center gap-2 px-3 py-1 bg-brand-accent/50 rounded-lg w-fit">
          <span className="text-[10px] font-bold uppercase tracking-widest opacity-40 italic">Reviewed:</span>
